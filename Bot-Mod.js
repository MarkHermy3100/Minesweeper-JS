...
function resetBot() {
    ...
    TARGET_BOARD = document.querySelector("#A43");
    const target_board_last_cell = TARGET_BOARD.children[TARGET_BOARD.children.length - 2].id;
    TARGET_BOARD_COLUMN = Number(target_board_last_cell.slice(5, 7)) + 1;
    TARGET_BOARD_ROW = Number(target_board_last_cell.slice(8, 10)) + 1;
    ...
    startBot(Math.max(100, TARGET_BOARD_ROW * TARGET_BOARD_COLUMN / 200));
}
...
function getCellNumber(cellOnScreen) {
    ...
    return Number(cellOnScreen.className.slice(cellOnScreen.className.length - 1));
}
...
function scanBoard(delay) {
    ...
    let scan = setInterval(() => {
        ...
        try {currentCell = target_board_data[Math.floor(index / (TARGET_BOARD_COLUMN + 1))][index % (TARGET_BOARD_COLUMN + 1)];}
        catch {return;};
        if (index % (TARGET_BOARD_COLUMN + 1) === TARGET_BOARD_COLUMN) {
            ++index;
            return;
        }
        ...
        if (TARGET_BOARD.children[index].className.includes("open")) {
            currentCell.number = getCellNumber(TARGET_BOARD.children[index]);
            //console.log(currentCell.x, currentCell.y, currentCell.number);
        }
        else if (TARGET_BOARD.children[index].className.includes("flag")) {
        ...
        if (index === TARGET_BOARD_ROW * (TARGET_BOARD_COLUMN + 1) - 1) {
            clearInterval(scan);
        }
        ++index;
    }, delay);
            ...
            if (cellGroup.length === i + 1) {
                for (let j = 0; j < cellGroup.length; ++j) {
                    if (j > 0 && !target_board_data[cellGroup[j].x][cellGroup[j].y].solved)
                        ...
                        rightClicker(TARGET_BOARD.children[cellGroup[j].x * (TARGET_BOARD_COLUMN + 1) + cellGroup[j].y]);
                            ...
                            if (j - i + 1 === cellGroupHigh.length - sharedCells.length) {
                                cellGroupHigh.filter(c => !sharedCells.includes(c)).forEach(cell => {
                                    if (!target_board_data[cell.x][cell.y].solved)
                                        ...
                                        rightClicker(TARGET_BOARD.children[cell.x * (TARGET_BOARD_COLUMN + 1) + cell.y]);
    ...
    for (let i = 0; i <= 8; ++i) {
        for (let i2 = 0; i2 < cellGroupDensity[i].length; ++i2) {
            ...
            if (i === cellGroup.filter(c => (c.solved && c.number === -1)).length) {
                ...
                clicker(TARGET_BOARD.children[cellGroup[0].x * (TARGET_BOARD_COLUMN + 1) + cellGroup[0].y]);
                        ...
                        if (sharedCells.length === Math.min(cellGroup1.length, cellGroup2.length) - 1) {
                            if (cellGroup1.length < cellGroup2.length) {
                                cellGroup2.filter(c => !sharedCells.includes(c)).forEach(cell => {
                                    ...
                                    clicker(TARGET_BOARD.children[cell.x * (TARGET_BOARD_COLUMN + 1) + cell.y]);
                                });
                            }
                            else {
